...
- `/add-feature` スキルによる設計レビュー→実装→3軸コードレビュー→コンパイル検証→PRの完全自動化
...
## Phase 5: `/add-feature` スキルで実装・レビュー・検証を完全自動化

### スキルとは

Claude Codeには「スキル」という仕組みがあり、`.claude/commands/` にMarkdownファイルを置くだけで、カスタムのスラッシュコマンドを定義できます。`/add-feature 002` のように呼び出すと、Markdownに記述された手順がClaude Codeに読み込まれ、全ステップを自動で実行します。

### スキルの進化: implement-game → add-feature

初期は `implement-game` というスキルで「コード生成→PR」だけを自動化していました。しかし運用を続ける中で、以下の課題が出てきました:

- 設計の抜け漏れに気づかないまま実装が進む
- コード品質にバラつきがある（レビューなしでマージ）
- Unity上でコンパイルエラーが出て手戻りする

そこで、**設計レビュー・3軸コードレビュー・Unityコンパイル検証**まで含むフルワークフローの `/add-feature` に進化させました。

### add-feature スキルの全体フロー（7ステップ）

```
/add-feature [ゲームID]
    ↓
① 情報収集: GitHub Issue + GameRegistry.json から仕様取得
    ↓
② 調査: 直近実装済みゲームのコードパターン確認、Context7 MCPで技術調査
    ↓
③ 計画: ステアリングファイル作成
   （requirements.md / design.md / tasklist.md）
    ↓
③.5 設計書のセルフチェック（チェックリスト形式）
    ↓
④ 実装ループ: tasklist.md の全タスクを1つずつ消化
   （C#スクリプト群 + SceneSetup + アセット + GameRegistry更新）
    ↓
⑤ 実装検証（2段階）:
   a. 2軸コードレビュー（構造/欠陥）→ 必須指摘0件まで修正
   b. Unity MCPでコンパイル検証 + SceneSetup自動実行
    ↓
⑥ ブランチ作成 → commit → push → PR作成 → mainマージ
    ↓
⑦ 振り返り記録 + GitHub Issue にコメント
```

### ステップ③.5: 設計書のセルフチェック

実装に入る前に、以下のチェックリストを自己確認します。サブエージェントは起動しません。

```
- [ ] design.md に namespace Game[ID]_[Title] が明記されているか
- [ ] 各クラスの GameManager 参照取得方法が明記されているか
- [ ] クリアとゲームオーバー両方の状態遷移フローが記述されているか
- [ ] SceneSetup で配線が必要なフィールドが全て列挙されているか
...
実装後、2つのサブエージェントを**並列起動**してコードレビューを実行します。

| サブエージェント | 観点 |
|---|---|
| `code-reviewer-structural` | 構造・アーキテクチャの妥当性 + 実装品質 |
| `code-reviewer-secondary` | 欠陥・セキュリティリスク |

`[必須]` の指摘がある場合のみ修正して先に進みます。

### ステップ⑤: Unityコンパイル検証

Unity MCP（Model Context Protocol）が接続されている場合、**Unity Editorと直接通信して**コンパイルエラーの有無を確認します。

```
# Unityプロジェクトをリフレッシュしてコンパイル
mcp__coplaydev-mcp__refresh_unity(compile="request", wait_for_ready=true)

# コンソールのエラーを確認
mcp__coplaydev-mcp__read_console(types=["error"], count=20)

# SceneSetupメニューを自動実行
mcp__coplaydev-mcp__execute_menu_item(menu_path="Assets/Setup/004 WordCrystal")
```

コンパイルエラーがあればスクリプトを修正して再実行。エラーゼロを確認してからPR作成に進みます。

### スキルに埋め込んだナレッジ

実装を繰り返す中でハマったポイントをスキル自体にルールとして組み込んでいます。
...
Claude Codeは毎回このルールを読み込んだ上でコード生成するため、同じハマりポイントを二度踏まず、品質が安定します。
...
`/add-feature` で1本のゲーム実装は自動化できましたが、100本を1本ずつ `/add-feature 004`、`/add-feature 005`... と打つのは非効率です。
...
next_id を読み、/add-feature [ID] を実行 → 完了後 next_id を +1 更新
...
## Phase 9: `/add-feature` スキルのサブエージェント最適化

### きっかけ: `/insights` でセッション分析

Claude Code には `/insights` コマンドがあり、過去のセッション履歴を分析してレポートを出力してくれます。

```
/insights
```

出力されたレポートには「wrong_approach（間違ったアプローチ）の発生件数」や「セッションの成功率」とともに、**改善提案**が含まれていました。その中に「1ゲームの実装で大量のサブエージェントが起動している」という指摘があり、スキルを見直すきっかけになりました。

### 問題: 1ゲームに最大9サブエージェントが起動していた

実際にスキルのコードを確認すると、**1ゲームの実装に最大9つのサブエージェントが起動する**ことがわかりました。

```
doc-reviewer          × 1  （ドキュメントレビュー）
implementation-validator × 1  （実装品質検証）
code-reviewer-structural × 1  （構造レビュー）
code-reviewer-secondary  × 1  （欠陥レビュー）
code-reviewer-docs       × 1  （API準拠レビュー）
再レビューループ          × 最大3  （C評価以下で最大2回繰り返し）
                         ────────
合計                     最大 9 エージェント
```

100本のゲームを量産するワークフローでは、エージェント起動のコストと時間が積み重なります。各エージェントが本当に必要かを見直しました。

### 削減した内容と理由

#### ① doc-reviewer → セルフチェックリストに置換

ミニゲームは独立した実装で、requirements.md / design.md の構造が毎回同じパターンです。30本以上実装した結果、**doc-reviewer から `[必須]` 指摘が出ることはほぼなく**なっていました。

代わりにチェックリスト5項目のセルフチェックに置換し、同じ品質保証をゼロコストで実現しました。

#### ② implementation-validator を削除（structural reviewerに統合）

`implementation-validator`（実装品質検証）と `code-reviewer-structural`（構造・アーキテクチャ検証）は観点が重複していました。structural reviewerのプロンプトに実装品質チェックを含めることで1エージェントに統合しました。

#### ③ code-reviewer-docs を削除

`code-reviewer-docs` は「外部APIの公式ドキュメント準拠」をチェックするエージェントです。しかしUnity内製ミニゲームは外部API依存がほぼなく、**毎回空振り**していました。ゲームを実装するたびに「準拠上の問題なし」というレポートが出るだけで、価値がありませんでした。

#### ④ フィードバックループを廃止

評価がC以下の場合に再レビューを最大2回繰り返す仕組みを廃止しました。再レビューは3エージェント × 最大2回 = 最大6エージェント追加消費です。ミニゲームの規模では1回のレビューで重要な問題は全て検出できており、**再レビューで新たな `[必須]` 指摘が出ることはありませんでした**。

#### ⑤ 完了条件を「B以上 + 必須0件」→「必須0件のみ」に緩和

A/B/C/D の総合評価スコアは主観的で、スコアをBに上げるために不要な修正が発生することがありました。本当に重要なのは `[必須]` 指摘のゼロ件です。スコア基準を廃止し、必須指摘のみを判断基準にしました。
...
|---|---|---|
| サブエージェント数 | 最大9 | **2** |
| ドキュメントレビュー | doc-reviewer（1エージェント） | セルフチェックリスト（0エージェント） |
| 実装品質検証 | implementation-validator（別エージェント） | structural reviewerに統合 |
| コードレビュー | 3軸（structural/secondary/docs） | **2軸（structural/secondary）** |
| フィードバックループ | 最大2回（最大+6エージェント） | **廃止** |
| 完了条件 | B以上 + 必須0件 | **必須0件のみ** |

コストと時間を大幅に削減しつつ、**品質チェックの本質（必須指摘の検出・修正）は維持**しています。

:::message
Claude Codeのスキルは「できることを全部入れる」より「本当に必要なものだけ残す」が大切です。実際に何十回と動かしてみて初めてわかる「空振りパターン」があります。
:::
...
1. GitHub Projects でゲームを選ぶ（工数Sでフィルター）
      ↓
2. Claude Code に「/add-feature 004」と依頼
      ↓
3. 自動で 設計レビュー → C#生成 → コードレビュー → コンパイル検証 → PR作成 → マージ
      ↓
...
- **`/add-feature` スキル** により、設計チェック→実装→コードレビュー→コンパイル検証→PRマージまでの全工程を1コマンドで自動化した
...
- **スキルの最適化** により、サブエージェント起動数を最大9→2に削減。「空振りパターン」は実運用でしか見えない
