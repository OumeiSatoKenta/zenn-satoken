...
① 情報収集: GitHub Issue + GameRegistry.json から仕様取得
    ↓
② 調査: 既存コードのパターン確認、Context7 MCPで技術調査
    ↓
③ 計画: ステアリングファイル作成
   （requirements.md / design.md / tasklist.md）
    ↓
③.5 ドキュメントレビュー ← doc-reviewer サブエージェント
   → 指摘があれば設計を修正してから実装へ
    ↓
④ 実装ループ: tasklist.md の全タスクを1つずつ消化
   （C#スクリプト群 + SceneSetup + アセット + GameRegistry更新）
    ↓
⑤ 実装検証（3段階）:
   a. implementation-validator で品質検証
   b. 3軸コードレビュー（構造/欠陥/API準拠）→ 総合評価B以上まで修正ループ
   c. Unity MCPでコンパイル検証 + SceneSetup自動実行
    ↓
...
実装に入る前に、設計ドキュメントをサブエージェントにレビューさせます。

```
doc-reviewer サブエージェント:
  レビュー対象: requirements.md, design.md
  観点:
    1. 完全性 — 必要な要件が網羅されているか
    2. 具体性 — 曖昧な表現がなく実装可能な粒度か
    3. 一貫性 — 既存アーキテクチャと矛盾がないか
    4. 実現可能性 — タスク分解が適切か
```

`[必須]` の指摘がある場合は設計を修正してから実装に進みます。これにより「設計の抜け漏れに実装中に気づく」手戻りを防いでいます。

### ステップ⑤: 3軸コードレビュー

実装後、3つのサブエージェントを**並列起動**してコードレビューを実行します。

| サブエージェント | 観点 |
|---|---|
| `code-reviewer-structural` | 構造・アーキテクチャの妥当性 |
| `code-reviewer-secondary` | 欠陥・セキュリティリスク |
| `code-reviewer-docs` | API使用の正確性・公式ドキュメント準拠 |

結果を統合して総合評価（A/B/C/D）を算出します。**C以下の場合は指摘を修正して再レビュー**（最大2回ループ）し、B以上になるまで先に進みません。
...
- **`/add-feature` スキル** により、設計レビュー→実装→3軸コードレビュー→コンパイル検証→PRマージまでの全工程を1コマンドで自動化した
